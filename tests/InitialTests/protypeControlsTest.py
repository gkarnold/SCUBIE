...
while keepLooping:
    ...
    # Gets the value of the right index button for the reverse function
    index_R = my_joystick.get_button(9)*100


    axesString = str(header)+'H'+str(v_L)+'L'+str(v_R)+'R'+str(trigger)+'V'+str(index_R)
    # axesString = str('100L10R53V')
    # Outputs the value of the left vertical controller axis for comparision
    print('Controller Axis (v_L, v_R, trigger, index_R): {}'.format(axesString))
