...
while keepLooping:
    ...
    # Gets the values of the index buttons for triggering reverse
    index_L = my_joystick.get_button(8)*100
    index_R = my_joystick.get_button(9)*100


    axesString = str(header)+'H'+str(v_L)+'L'+str(v_R)+'R'+str(trigger)+'V'+str(index_L)+'Q'+str(index_R)+'W'
    # axesString = str('100L10R53V')
    # Outputs the value of the left vertical controller axis for comparision
    print('Controller Axis (v_L, v_R, trigger, index_L, index_R): {}'.format(axesString))
    # Sends the value of the left vertical controller axis to Arduino
